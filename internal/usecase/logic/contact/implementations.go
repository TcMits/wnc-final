...
func (s *CustomerContactListUseCase) List(ctx context.Context, limit, offset *int, o *model.ContactOrderInput, w *model.ContactWhereInput) ([]*model.Contact, error) {
	...
	if err != nil {
		return nil, usecase.WrapError(fmt.Errorf("internal.usecase.logic.contact.implementations.CustomerContactListUseCase.List: %s", err))
	}
	return entites, nil
}
func (s *CustomerContactDeleteUseCase) Delete(ctx context.Context, e *model.Contact) error {
	...
	if err != nil {
		return usecase.WrapError(fmt.Errorf("internal.usecase.logic.contact.implementations.CustomerContactDeleteUseCase.Delete: %s", err))
	}
	return nil
}
...
func (s *CustomerContactGetFirstMineUseCase) GetFirstMine(ctx context.Context, o *model.ContactOrderInput, w *model.ContactWhereInput) (*model.Contact, error) {
	entites, err := s.UC1.ListMine(ctx, generic.GetPointer(1), generic.GetPointer(0), o, w)
	if err != nil {
		return nil, err
	}
	if len(entites) > 0 {
...
func (s *CustomerContactCreateUseCase) Create(ctx context.Context, i *model.ContactCreateInput) (*model.Contact, error) {
	...
	if err != nil {
		return nil, usecase.WrapError(fmt.Errorf("internal.usecase.logic.contact.implementations.CustomerContactCreateUseCase.Create: %s", err))
	}
	return e, nil
}
func (s *CustomerContactUpdateUseCase) Update(ctx context.Context, e *model.Contact, i *model.ContactUpdateInput) (*model.Contact, error) {
	...
	if err != nil {
		return nil, usecase.WrapError(fmt.Errorf("internal.usecase.logic.contact.implementations.CustomerContactUpdateUseCase.Update: %s", err))
	}
	return e, nil
}
func (s *CustomerContactValidateCreateInputUseCase) ValidateCreate(ctx context.Context, i *model.ContactCreateInput) (*model.ContactCreateInput, error) {
	...
	if i.BankName == "" {
		i.BankName = *s.UC2.GetProductOwnerName()
	}
	e, err := s.UC1.GetFirstMine(ctx, nil, &model.ContactWhereInput{
	...
	user := usecase.GetUserAsCustomer(ctx)
	target, err := s.UC3.GetFirst(ctx, nil, &model.CustomerWhereInput{
	...
	if target == nil {
		...
		if err != nil {
			return nil, usecase.WrapError(fmt.Errorf("internal.usecase.logic.contact.implementations.CustomerContactValidateCreateInputUseCase.ValidateCreate: %s", err))
		}
		if r == nil {
